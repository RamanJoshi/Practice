...
class UserController extends AbstractS2UiController {
	...
	def userCache
	def mailService;
	def create() {	println("it is running123487897")
		def user = lookupUserClass().newInstance(params)
		[user: user, authorityList: sortedRoles()]
	}
	def save() {
		println("it is running1234")
		def user = lookupUserClass().newInstance(params)
		if (params.password) {
...
//		grailsApplication.config.grails.mail.host = "smtp.mandrillapp.com"
//		grailsApplication.config.grails.mail.port = 587
//		grailsApplication.config.grails.mail.username = "arif.khan@oodlestechnologies.com"
//		grailsApplication.config.grails.mail.password = "E6s4mcmfBBdJhwYKlC6d0A"
//		grailsApplication.config.grails.mail.props = ["mail.smtp.auth":"true",
		//	"mail.smtp.socketFactory.port":"587"]
		println 'mail '+grailsApplication.config.grails.mail.port
		mailService.sendMail {
			to user.username
			from "v360telehealth@gmail.com"
	...
	def sendEMail(){
		mailService.sendMail {
			to "raman.joshi@oodlestechnologies.com"
			from "v360telehealth@gmail.com"
			subject "Telehealth Alert"
			body "how are you"
		}
